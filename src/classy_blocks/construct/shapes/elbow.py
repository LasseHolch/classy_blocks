...
class SemiElbow(RoundSolidShape):
    """A curved round shape of varying cross-section"""
    ...
    def __init__(
        ...
        radius_1 = f.norm(np.asarray(radius_point_1) - np.asarray(center_point_1))
        sketch_1 = HalfDisk(center_point_1, radius_point_1, normal_1)
        radius_ratio = radius_2 / radius_1
    ...
    def chain(
        ...
        """Use another round Shape's end face as a starting point for this SemiElbow;
        Returns a new SemiElbow object. To start from the other side, use start_face = True"""
        if not isinstance(source.sketch_1, HalfDisk):
            raise ElbowCreationError(
                "`chain()` operation failed: expecting `HalfDisk`-type face",
                f"Given `{type(source.sketch_1)}` face instance",
            )
